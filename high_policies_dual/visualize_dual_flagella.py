...
def parse_args():
    parser = argparse.ArgumentParser(description="Dual-Flagella Dual Independent-Policy Visualizer")
...
def find_latest_checkpoint(base_dir=None):
    ...
    policy_roots = [item for item in base_dir.iterdir() if item.is_dir() and item.name.startswith("dual_policy_")]
    if not policy_roots:
        return None
...
def render_frame(
    ...
    ax.set_title("Dual-Policy Flagella Chemotaxis Visualization")
