...
parser = argparse.ArgumentParser(description="Train triple-flagella triple independent-policy senior policy")
...
POLICY_DIR = os.path.join(os.getcwd(), f"triple_policy_{TIMESTAMP}")
...
class TrainingMetricsCallback(DefaultCallbacks):
    def on_episode_end(self, *, worker, base_env, policies, episode, env_index, **kwargs):
        ...
        rewards = getattr(env_ref, "last_robot_rewards", [0.0, 0.0, 0.0])
        orders = getattr(env_ref, "last_robot_orders", [1, 1, 1])
        for i, tag in enumerate(["robot_1", "robot_2", "robot_3"]):
            episode.custom_metrics[f"{tag}_reward"] = float(rewards[i])
            episode.custom_metrics[f"{tag}_order"] = float(orders[i])
        ...
        episode.custom_metrics["robot_3_concentration"] = float(getattr(env_ref, "con3", 0.0))
        for i, (attr, tag) in enumerate([("last_centroid1", "robot_1"), ("last_centroid2", "robot_2"), ("last_centroid3", "robot_3")]):
            c = getattr(env_ref, attr, [0.0, 0.0])
            episode.custom_metrics[f"{tag}_centroid_x"] = float(c[0])
            episode.custom_metrics[f"{tag}_centroid_y"] = float(c[1])
...
def write_training_scalars(writer, result, iteration):
    ...
    for policy_id in (POLICY_ID_R1, POLICY_ID_R2, POLICY_ID_R3):
        learner_info = result.get("info", {}).get("learner", {}).get(policy_id, {})
        for key in ("learner_stats", "stats"):
            stats = learner_info.get(key, {})
            if not isinstance(stats, dict):
                continue
            for name, value in stats.items():
                maybe_add_scalar(writer, f"learner/{policy_id}/{name}", value, iteration)
...
def build_ppo_config(cli_args):
    ...
    config["min_sample_timesteps_per_iteration"] = 300
...
def write_training_run_markdown(run_dir, cli_args, trainer_config, visualizer_snapshot_path, swimmer_snapshot_path):
    env_params = {
        ...
        "robot3_init": swimmer_module.ROBOT3_INIT,
        "macro_horizon": swimmer_module.MACRO_HORIZON,
        "low_level_hold_steps": swimmer_module.LOW_LEVEL_HOLD_STEPS,
        ...
        "robot_ids": ROBOT_IDS,
        "policy_ids": [POLICY_ID_R1, POLICY_ID_R2, POLICY_ID_R3],
        "observation_space": "Discrete(6) — current strategy order",
        "action_space": "Discrete(3) — relative order change (-1, 0, +1)",
    ...
    lines = [
        "# Triple Independent-Policy Training Run Parameters",
        "",
