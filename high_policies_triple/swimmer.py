...
class swimmer_gym(MultiAgentEnv):
    ...
    def __init__(self, env_config):
        ...
        self.last_robot_rewards = [0.0, 0.0, 0.0]
        self.last_robot_orders = [1, 1, 1]
        ...
        self._last_ll_action3 = np.zeros((ENV_LINK_NUM - 1,), dtype=np.float64)
    ...
    def _build_initial_geometry(self):
        ...
        self.state3, self.Xfirst3, self.XY_positions3 = self._build_initial_robot_state(ROBOT3_INIT)
        ...
        self.con3 = float(np.mean(compute_concentration(self.XY_positions3)))

        self.aprm1 = coarse_select_order(self.XY_positions1)
        self.aprm2 = coarse_select_order(self.XY_positions2)
        self.aprm3 = coarse_select_order(self.XY_positions3)
        self.order1 = self.aprm1
        self.order2 = self.aprm2
        self.order3 = self.aprm3

        self.last_robot_rewards = [0.0, 0.0, 0.0]
        self.last_robot_orders = [self.order1, self.order2, self.order3]
    ...
    def _compute_low_level_action(self, robot_idx, strategy_order):
        ...
        # 2. 取原始铰链角
        if robot_idx == 0:
            raw_hinges = self.state1[3:].copy()
        elif robot_idx == 1:
            raw_hinges = self.state2[3:].copy()
        else:
            raw_hinges = self.state3[3:].copy()
        ...
        # 6. 关节限位检查
        target_state = [self.state1, self.state2, self.state3][robot_idx]
        return self._sanitize_low_level_action(target_state, action)

    def _apply_triple_solver(self, action1, action2, action3):
        (
            state1_next, _xn1, _yn1, _r1, x_first_delta1, x_positions1, y_positions1,
            state2_next, _xn2, _yn2, _r2, x_first_delta2, x_positions2, y_positions2,
            state3_next, _xn3, _yn3, _r3, x_first_delta3, x_positions3, y_positions3,
            _pressure_diff, _pressure_end, _pressure_all,
        ) = RK_triple(
            self.state1, action1, self.Xfirst1,
            self.state2, action2, self.Xfirst2,
            self.state3, action3, self.Xfirst3,
        )

        self.state1 = state1_next.copy()
        self.state2 = state2_next.copy()
        self.state3 = state3_next.copy()
        self.Xfirst1 = self.Xfirst1 + x_first_delta1
        self.Xfirst2 = self.Xfirst2 + x_first_delta2
        self.Xfirst3 = self.Xfirst3 + x_first_delta3
        ...
        self.XY_positions3 = np.concatenate((np.array(x_positions3).reshape(-1, 1), np.array(y_positions3).reshape(-1, 1)), axis=1)
        ...
        self.state3[0] = centroid3[0]
        self.state3[1] = centroid3[1]
    ...
    def _capture_substep_frame(self, substep_index):
        ...
        return {
            ...
            "xy3": np.array(self.XY_positions3, copy=True),
            "state1": np.array(self.state1, copy=True),
            "state2": np.array(self.state2, copy=True),
            "state3": np.array(self.state3, copy=True),
            ...
            "ll_action3": np.array(self._last_ll_action3, copy=True),
            "xfirst1": np.array(self.Xfirst1, copy=True),
            "xfirst2": np.array(self.Xfirst2, copy=True),
            "xfirst3": np.array(self.Xfirst3, copy=True),
        }
    ...
    def _record_macro_step(self):
        ...
        combined_state = np.concatenate((self.state1.copy(), self.state2.copy(), self.state3.copy()), axis=0)
        summary_row = np.array(
            [
                self.state1[0], self.state1[1],
                self.state2[0], self.state2[1],
                self.state3[0], self.state3[1],
                self.last_robot_rewards[0], self.last_robot_rewards[1], self.last_robot_rewards[2],
                float(self.order1), float(self.order2), float(self.order3),
    ...
    def step(self, action_dict):
        ...
        for substep_index in range(self.low_level_hold_steps):
            ...
            action3 = self._compute_low_level_action(2, self.order3)
            self._last_ll_action1 = action1
            self._last_ll_action2 = action2
            self._last_ll_action3 = action3
            self._apply_triple_solver(action1, action2, action3)
        ...
        con3_new = float(np.mean(compute_concentration(self.XY_positions3)))
        reward1 = (con1_new - self.con1) * CON_REWARD_SCALE
        reward2 = (con2_new - self.con2) * CON_REWARD_SCALE
        reward3 = (con3_new - self.con3) * CON_REWARD_SCALE
        self.con1 = con1_new
        self.con2 = con2_new
        self.con3 = con3_new
        ...
        self.last_robot_rewards = [reward1, reward2, reward3]
        ...
        print(
            ...
            f"R1: o={self.order1}({STRATEGY_NAMES[self.order1]}), rwd={reward1:>8.4f}, con={self.con1:>8.4f} | "
            f"R2: o={self.order2}({STRATEGY_NAMES[self.order2]}), rwd={reward2:>8.4f}, con={self.con2:>8.4f} | "
            f"R3: o={self.order3}({STRATEGY_NAMES[self.order3]}), rwd={reward3:>8.4f}, con={self.con3:>8.4f}"
        )
        ...
        rewards = {
            ...
            ROBOT_IDS[2]: float(reward3),
        }
        dones = {robot_id: self.done for robot_id in ROBOT_IDS}
        dones["__all__"] = self.done
        infos = {
            ROBOT_IDS[0]: {"reward": float(reward1), "concentration": float(self.con1), "aprm": int(self.aprm1), "order": int(self.order1), "strategy": STRATEGY_NAMES[self.order1]},
            ROBOT_IDS[1]: {"reward": float(reward2), "concentration": float(self.con2), "aprm": int(self.aprm2), "order": int(self.order2), "strategy": STRATEGY_NAMES[self.order2]},
            ROBOT_IDS[2]: {"reward": float(reward3), "concentration": float(self.con3), "aprm": int(self.aprm3), "order": int(self.order3), "strategy": STRATEGY_NAMES[self.order3]},
        }
