...
POLICY_ID_R3 = "policy_robot_3"

POLICY_MAP = {"robot_1": POLICY_ID_R1, "robot_2": POLICY_ID_R2, "robot_3": POLICY_ID_R3}
...
def parse_args():
    parser = argparse.ArgumentParser(description="Triple-Flagella Triple Independent-Policy Visualizer")
...
def find_latest_checkpoint(base_dir=None):
    ...
    policy_roots = [item for item in base_dir.iterdir() if item.is_dir() and item.name.startswith("triple_policy_")]
    if not policy_roots:
        return None
...
def build_config():
    ...
    config["multiagent"] = {
        "policies": {
            pid: (None, env_stub.observation_space, env_stub.action_space, {})
            for pid in (POLICY_ID_R1, POLICY_ID_R2, POLICY_ID_R3)
        },
        "policy_mapping_fn": policy_mapping_fn,
        "policies_to_train": [POLICY_ID_R1, POLICY_ID_R2, POLICY_ID_R3],
...
def capture_env_frame(env, substep_index):
    ...
    return {
        ...
        "ll_action3": np.array(env._last_ll_action3, copy=True),
        "xfirst1": np.array(env.Xfirst1, copy=True),
        "xfirst2": np.array(env.Xfirst2, copy=True),
        "xfirst3": np.array(env.Xfirst3, copy=True),
    }
...
def render_frame(
    ...
    centroid3 = np.array(frame["centroid3"], copy=True)
    if append_trace:
        trace1.append(centroid1)
        trace2.append(centroid2)
        trace3.append(centroid3)

    ax.clear()

    all_cx = (centroid1[0] + centroid2[0] + centroid3[0]) / 3.0
    all_cy = (centroid1[1] + centroid2[1] + centroid3[1]) / 3.0
    vr = ARGS.view_range
    grid_x = np.linspace(all_cx - vr, all_cx + vr, 120)
    grid_y = np.linspace(all_cy - vr, all_cy + vr, 120)
    ...
    if "flow_data" in frame:
        ...
        q_n = 32
        qx = np.linspace(all_cx - vr * 0.95, all_cx + vr * 0.95, q_n)
        qy = np.linspace(all_cy - vr * 0.95, all_cy + vr * 0.95, q_n)
    ...
    ax.plot(frame["xy3"][:, 0], frame["xy3"][:, 1], color="tab:green", linewidth=2.5)
    ax.scatter([centroid1[0]], [centroid1[1]], color="tab:blue", s=40)
    ax.scatter([centroid2[0]], [centroid2[1]], color="tab:red", s=40)
    ax.scatter([centroid3[0]], [centroid3[1]], color="tab:green", s=40)

    ax.scatter([CCENTER_X], [CCENTER_Y], color="gold", s=150, marker="*", edgecolors="black", zorder=10)

    if len(trace1) > 1:
        for tr, col in [(trace1, "tab:blue"), (trace2, "tab:red"), (trace3, "tab:green")]:
            tr_np = np.array(tr)
            ax.plot(tr_np[:, 0], tr_np[:, 1], color=col, alpha=0.5, linewidth=1.0)

    for state_key, centroid, color in [("state1", centroid1, "tab:olive"), ("state2", centroid2, "tab:olive"), ("state3", centroid3, "tab:olive")]:
        heading = compute_average_heading(frame[state_key])
        draw_heading(ax, centroid, heading, color)

    ax.set_xlim(all_cx - ARGS.view_range, all_cx + ARGS.view_range)
    ax.set_ylim(all_cy - ARGS.view_range, all_cy + ARGS.view_range)
    ...
    info_text = "\n".join(
        [
            ...
            f"R1 strategy: {strategy_triple[0]}",
            f"R1 rwd: {robot_rewards[0]:.4f}, con: {robot_concentrations[0]:.4f}",
            f"R2 strategy: {strategy_triple[1]}",
            f"R2 rwd: {robot_rewards[1]:.4f}, con: {robot_concentrations[1]:.4f}",
            f"R3 strategy: {strategy_triple[2]}",
            f"R3 rwd: {robot_rewards[2]:.4f}, con: {robot_concentrations[2]:.4f}",
    ...
    ax.set_title("Triple-Policy Flagella Chemotaxis Visualization")
...
def main():
    ...
    if checkpoint is None:
        print("[Error] No triple-policy checkpoint found. Run train.py first or pass --checkpoint.")
        sys.exit(1)

    print(f"Loading triple-policy checkpoint: {checkpoint}")
    ...
    policy_r3 = agent.get_policy(POLICY_ID_R3)
    if policy_r1 is None or policy_r2 is None or policy_r3 is None:
        raise RuntimeError(f"Could not load all three policies from checkpoint. "
                           f"R1={policy_r1 is not None}, R2={policy_r2 is not None}, R3={policy_r3 is not None}")
    print(">>> Triple-policy checkpoint restore succeeded.")

    # ================= 阶段 1：预计算所有 macro step =================
    print(f">>> Precomputing {ARGS.steps} macro steps (this may take a while)...")
    policy_map = {"robot_1": policy_r1, "robot_2": policy_r2, "robot_3": policy_r3}
    all_packages = []
    for i in range(ARGS.steps):
        action_dict = {}
        for robot_id in ROBOT_IDS:
            action_id = compute_agent_action(policy_map[robot_id], obs_dict[robot_id], explore=ARGS.explore)
        ...
        strategy_triple = (STRATEGY_NAMES[env.order1], STRATEGY_NAMES[env.order2], STRATEGY_NAMES[env.order3])

        package = {
            "frames": frames,
            "strategy_triple": strategy_triple,
            "robot_rewards": list(env.last_robot_rewards),
            "robot_concentrations": [env.con1, env.con2, env.con3],
            "robot_orders": [env.order1, env.order2, env.order3],
        }
        all_packages.append(package)

        print(
            f"  [{i + 1:>4d}/{ARGS.steps}] "
            f"R1: aprm={env.aprm1}->order={env.order1}({strategy_triple[0]}), rwd={env.last_robot_rewards[0]:>8.2f}, con={env.con1:.4f} | "
            f"R2: aprm={env.aprm2}->order={env.order2}({strategy_triple[1]}), rwd={env.last_robot_rewards[1]:>8.2f}, con={env.con2:.4f} | "
            f"R3: aprm={env.aprm3}->order={env.order3}({strategy_triple[2]}), rwd={env.last_robot_rewards[2]:>8.2f}, con={env.con3:.4f}"
        )
            ...
            if "ll_action1" in frame and "xfirst1" in frame:
                try:
                    flow_data = compute_stokeslet_forces(
                        ...
                        frame["state3"], frame["ll_action3"], frame["xfirst3"],
                    )
    ...
    try:
        for macro_index, package in enumerate(all_packages, start=1):
            strategy_triple = package["strategy_triple"]
