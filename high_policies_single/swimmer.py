...
class swimmer_gym(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        # 动作：Discrete(9) = 联合动作，action//3 为 R1 的 aadj, action%3 为 R2 的 aadj
        self.action_space = spaces.Discrete(9)
        # 观测：Box(12,) = 两个机器人 aprm 的 one-hot 拼接
    ...
    def _get_obs(self):
        """观测 = 两个机器人 aprm 的 one-hot 拼接 (12-dim)"""
        oh1 = np.zeros(NUM_STRATEGIES, dtype=np.float32)
        oh1[self.aprm1] = 1.0
        oh2 = np.zeros(NUM_STRATEGIES, dtype=np.float32)
        oh2[self.aprm2] = 1.0
        return np.concatenate([oh1, oh2])
    ...
    def step(self, action):
        ...
        # ========== 论文两步法：粗选 + RL 微调 ==========
        # 联合动作解码：action ∈ {0..8}, action//3 → R1 aadj, action%3 → R2 aadj
        action = int(action)
        aadj1 = action // 3 - 1   # {-1, 0, +1}
        aadj2 = action % 3 - 1    # {-1, 0, +1}
        ...
        obs = self._get_obs()
        reward = float(reward1) + float(reward2)
        info = {
            "robot_1_reward": float(reward1),
            "robot_2_reward": float(reward2),
            "robot_1_concentration": float(self.con1),
            "robot_2_concentration": float(self.con2),
            "robot_1_aprm": int(self.aprm1),
            "robot_2_aprm": int(self.aprm2),
            "robot_1_order": int(self.order1),
            "robot_2_order": int(self.order2),
            "robot_1_strategy": STRATEGY_NAMES[self.order1],
            "robot_2_strategy": STRATEGY_NAMES[self.order2],
        }
