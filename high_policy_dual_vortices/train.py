...
parser = argparse.ArgumentParser(description="Train dual-flagella dual independent-policy senior policy in cellular vortex flow")
...
POLICY_DIR = os.path.join(os.getcwd(), f"vortex_dual_policy_{TIMESTAMP}")
...
def write_training_scalars(writer, result, iteration):
    ...
    for policy_id in (POLICY_ID_R1, POLICY_ID_R2):
...
def build_ppo_config(cli_args):
    ...
    config["min_sample_timesteps_per_iteration"] = 800
...
def write_training_run_markdown(run_dir, cli_args, trainer_config, visualizer_snapshot_path, swimmer_snapshot_path):
    env_params = {
        ...
        "vortex_strength": swimmer_module.VORTEX_STRENGTH,
        "vortex_cell_L": swimmer_module.VORTEX_CELL_L,
        "visualizer_snapshot": visualizer_snapshot_path,
        "swimmer_snapshot": swimmer_snapshot_path,
        "robot_ids": ROBOT_IDS,
        "policy_ids": [POLICY_ID_R1, POLICY_ID_R2],
    ...
    lines = [
        "# Dual Independent-Policy + Cellular Vortex Training Run Parameters",
        "",
